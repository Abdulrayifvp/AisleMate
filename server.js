...
// const firstQuery = readlineSync.question('Enter Your Query: ')
...
async function main() {
  console.log(colors.bold.green("Welcome to Supermarket Chatbot"));
  console.log(colors.bold.green("Please enter your query"));
  
  while(true){
    const query = readlineSync.question(colors.yellow("You: "));
    if(query === "exit"){
      return
    }
    loadSupermarketData().then(data =>
      embbedData(data).then((ed) => {
        ...
        model.generateContent(
          "if the prompt specify a product then return that specific product or if the prompt specify a recipe or a plan then return a array of products to fullfil the need - prompt :" + query,
        ).then((result) => {
